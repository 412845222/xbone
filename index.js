...
export class Xbone {
  ...
  gridDataRelative(index, griding_info = null) {
    ...
    reload_point = new Victor(reload_point.x * this.relative_scale, reload_point.y * this.relative_scale);
    ...
    let relative_img_point_move = new Victor(relative_point.x - img_point.x * relative_scale, relative_point.y - img_point.y * relative_scale);

    let boneList_relative = [];
    boneList.forEach((bone) => {
      let temp_bone = [null, null, null];
      temp_bone[0] = new Victor(bone[0].x * relative_scale + relative_img_point_move.x, bone[0].y * relative_scale + relative_img_point_move.y);
      temp_bone[1] = new Victor(bone[1].x * relative_scale + relative_img_point_move.x, bone[1].y * relative_scale + relative_img_point_move.y);
      temp_bone[2] = new Victor(bone[2].x * relative_scale + relative_img_point_move.x, bone[2].y * relative_scale + relative_img_point_move.y);
      boneList_relative.push(temp_bone);
    });

    let gridingPoint_relative = [];
    gridingPoint.forEach((point) => {
      let temp_point = new Victor(point.x * relative_scale, point.y * relative_scale);
  ...
  getParentBoneRotatePoint(point, boneList, b_idx, img_point, angleList) {
    ...
    for (let i = 1; i <= b_idx; i++) {
      let bone = boneList[i];
      bone = [new Victor(bone[0].x, bone[0].y), new Victor(bone[1].x, bone[1].y), new Victor(bone[2].x, bone[2].y)];
      let point = retrun_point;
      let init_angle = 0;
      let angle = angleList[i];
      let new_point = this.replaceRotatePoint(angle + init_angle, bone, point, img_point);
  ...
  setBoneMoved_angle(info, index, boneIndex, angle) {
    ...
    initBone = [new Victor(initBone[0].x, initBone[0].y), new Victor(initBone[1].x, initBone[1].y), new Victor(initBone[2].x, initBone[2].y)];
    let img_point = info.img_point;
    let initBone_angle = info.initBone_angle;
    let init_angle = initBone_angle[boneIndex];

    let rotateBone = boneList[boneIndex];
    rotateBone = [new Victor(rotateBone[0].x, rotateBone[0].y), new Victor(rotateBone[1].x, rotateBone[1].y), new Victor(rotateBone[2].x, rotateBone[2].y)];
    if (boneIndex > 0) {
      let pre_bone = boneList[boneIndex - 1];
      pre_bone = [new Victor(pre_bone[0].x, pre_bone[0].y), new Victor(pre_bone[1].x, pre_bone[1].y), new Victor(pre_bone[2].x, pre_bone[2].y)];
      ...
      for (let i = 0; i < boneIndex; i++) {
        ...
        let bone = boneList[i];
        new_point = this.replaceRotateBonePoint(angle, bone, new_point, img_point);
      }
